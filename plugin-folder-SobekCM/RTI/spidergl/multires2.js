...
function createRtiViewer(idDiv, imageUrl, width, height) {
    var source_base_url = location.protocol + "//" + location.host + '/plugins/RTI/';
	...
	divHelp.innerHTML = '<div id = "guideTable"><div id = "guideCell"> <div id = "guideList"><h3>WebRTIViewer<br/></h3><ul><li>Pan: LeftMouseButton + MouseMove.</li><li>Zoom in: MouseWhell or press the button <span><img src = "http://localhost:52468/plugins/RTI/css/icons/zoomin.png" alt=""/></span></li><li>Zoom out: MouseWheel or press the button <span><img src = "' + source_base_url + 'css/icons/zoomout.png" alt=""/></span></li><li>To change the light direction: activate the light with the button <span><img src = "' + source_base_url + 'css/icons/light.png" alt=""/></span> and press LeftMouseButton + MouseMove or Ctrl + LeftMouseButton + MouseMove.</li><li>Fullscreen: press the button <span><img src = "' + source_base_url + 'css/icons/full.png" alt=""/></span> to active the fullscreen mode and the button <span><img src = "' + source_base_url + 'css/icons/full_on.png" alt=""/></span> to exit</li><li>To reset the viewpoint: press R.</li></ul><a href="http://vcg.isti.cnr.it/rti/webviewer.php" target = "_black">Visual Computing Lab ISTI CNR Pisa</a></div></div></div>'
...
function MultiResRenderer(gl, cacheSizeInBytes, onLoad)
 {
	...
	// box rendering
	/******************************************************/
	var bvs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/box.v.glsl");
	var bfs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/box.f.glsl");
...
MultiResRenderer.prototype = {
	...
	loadImage: function(url)
	{
		...
		{
			case 1:
				var vs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/multi.v.glsl");
				var fs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/hsh.f.glsl");
			...
			case 2:
				var vs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/multi.v.glsl");
				var fs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/lrgbptm.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
			case 3:
				var vs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/multi.v.glsl");
				var fs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/rgbptm.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
			case 4:
				var vs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/multi.v.glsl");
				var fs = sglLoadFile("http://localhost:52468/plugins/RTI/spidergl/image.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program  = prg;
				break;
